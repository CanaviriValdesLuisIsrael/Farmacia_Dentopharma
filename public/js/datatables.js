 ...
 *   https://datatables.net/download/#bs4/dt-2.3.8
 *
 * Included libraries:
 *   DataTables 2.3.8
 */
...
(function (factory) {
            ...
            while (cell) {
                if (
                    ...
                    if (write) {
                        ...
                        if (
                            $("div.dt-column-" + headerFooter, cell).length ===
                            0
                        ) {
                            $("<div>")
                        ...
                        fixed: Object.keys(column.searchFixed).map(
                            function (name) {
                                return {
                                    name: name,
                                    term:
                                        typeof column.searchFixed[name] !==
                                        "function"
                                            ? column.searchFixed[
                                                  name
                                              ].toString()
        ...
        if (longestData.length) {
            for (i = 0; i < longestData[0].length; i++) {
                ...
                for (j = 0; j < visibleColumns.length; j++) {
                    ...
                    var padding =
                        column.sContentPadding || (scrollX ? "-" : "");
    ...
    function _fnGetWideStrings(settings, colIdx) {
        ...
        if (!column.wideStrings) {
            ...
            for (
                var i = 0, iLen = settings.aiDisplayMaster.length;
                i < iLen;
                i++
            ) {
                ...
                var noHtml = _stripHtml(cellString, " ").replace(
                    /&nbsp;/g,
                    " ",
                );
    ...
    function __mlHelper(localeString) {
        return function (from, to, locale, def) {
            ...
            return function (d, type) {
                ...
                let options = {};
                let tzMatch =
                    typeof d === "string" ? d.match(__reIsoTimezone) : null;
